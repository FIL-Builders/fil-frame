  ...
  const handleRemoveFile = (fileName: string) => {
    setSelectedFiles(prevFiles => {
      const updatedFiles = prevFiles.filter(
        file => (file as unknown as { name: string }).name !== fileName
      );
  ...
  const handleUploadFiles = async (bucketName: string) => {
    ...
    for (const file of selectedFiles) {
      ...
      setUploadProgress(prev => ({ 
        ...prev, 
        [fileName]: { loading: true, completed: false }
      }));
      
      const success = await uploadFile(bucketName, file);
      
      // Update completion state
      setUploadProgress(prev => ({ 
        ...prev, 
        [fileName]: { loading: false, completed: success }
      }));
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <div className="flex justify-between items-center mb-8 border-b-2 border-black pb-4">
        <h1 className="text-4xl font-bold">Buckets</h1>
        <button
      ...
      <div className="space-y-4">
        {buckets.length === 0 && !isLoading ? (
          ...
          buckets.map((bucket) => (
            <div key={bucket.ID} className="bg-white rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
              {/* Bucket Header */}
              <div 
              ...
              >
                <div className="flex items-center space-x-4">
                  <ArchiveBoxIcon className="h-8 w-8 text-black" />
                  <div>
