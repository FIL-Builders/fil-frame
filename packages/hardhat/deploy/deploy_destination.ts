...
const deployYourContract: DeployFunction = async function (
  ...
  */
  ...
  await deploy("OnRampDestination", {
    from: deployer,
  ...
  const onRampDestination = await hre.ethers.getContract<Contract>(
    "OnRampDestination",
    deployer
  );
  console.log("👋 Deployed OnRampDestination at:", onRampDestination.address);
};

export default deployYourContract;

// Tags are useful if you have multiple deploy files and only want to run one of them.
// e.g. yarn deploy --tags OnRampDestination
deployYourContract.tags = ["OnRampDestination"];
